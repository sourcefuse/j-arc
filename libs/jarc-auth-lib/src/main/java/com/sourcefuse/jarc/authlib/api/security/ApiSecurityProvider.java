...
public class ApiSecurityProvider {
	...
	private List<Filter> getMiddlewaresFromConfig(HttpSecurity http) throws Exception {
	...
	public void processContentSecurityPolicy(List<Filter> middlewares) {
		if (apiSecurityConfig.enableContentSecurityPolicy() instanceof Boolean) {
			boolean optionValue = (boolean) apiSecurityConfig.enableContentSecurityPolicy();
			if (optionValue) {
				middlewares.add(new ContentSecurityPolicy());
			}
		} else {
			ContentSecurityPolicyConfig options = objectMapper
					.convertValue(apiSecurityConfig.enableContentSecurityPolicy(), ContentSecurityPolicyConfig.class);
			middlewares.add(new ContentSecurityPolicy(options));
		}
	}

	public void processCrossOriginEmbedderPolicy(List<Filter> middlewares) {
		if (apiSecurityConfig.enableCrossOriginEmbedderPolicy() instanceof Boolean) {
			boolean optionValue = (boolean) apiSecurityConfig.enableCrossOriginEmbedderPolicy();
			if (optionValue) {
				middlewares.add(new CrossOriginEmbedderPolicy());
			}
		} else {
			CrossOriginEmbedderPolicyConfig options = objectMapper.convertValue(
					apiSecurityConfig.enableCrossOriginEmbedderPolicy(), CrossOriginEmbedderPolicyConfig.class);
	...
	public void processCrossOriginOpenerPolicy(List<Filter> middlewares) {
		if (apiSecurityConfig.enableCrossOriginOpenerPolicy() instanceof Boolean) {
			boolean optionValue = (boolean) apiSecurityConfig.enableCrossOriginOpenerPolicy();
			if (optionValue) {
				middlewares.add(new CrossOriginOpenerPolicy());
			}
		} else {
			CrossOriginOpenerPolicyConfig options = objectMapper.convertValue(
					apiSecurityConfig.enableCrossOriginOpenerPolicy(), CrossOriginOpenerPolicyConfig.class);
			middlewares.add(new CrossOriginOpenerPolicy(options));
		}
	}

	public void processCrossOriginResourcePolicy(List<Filter> middlewares) {
		if (apiSecurityConfig.enableCrossOriginResourcePolicy() instanceof Boolean) {
			boolean optionValue = (boolean) apiSecurityConfig.enableCrossOriginResourcePolicy();
			if (optionValue) {
				middlewares.add(new CrossOriginResourcePolicy());
			}
		} else {
			CrossOriginResourcePolicyConfig options = objectMapper.convertValue(
					apiSecurityConfig.enableCrossOriginResourcePolicy(), CrossOriginResourcePolicyConfig.class);
			middlewares.add(new CrossOriginResourcePolicy(options));
		}
	}

	public void processOriginAgentCluster(List<Filter> middlewares) {
		if (apiSecurityConfig.enableOriginAgentCluster() == null || apiSecurityConfig.enableOriginAgentCluster()) {
	...
	public void processReferrerPolicy(List<Filter> middlewares) {
		if (apiSecurityConfig.enableReferrerPolicy() instanceof Boolean) {
			...
			ReferrerPolicyConfig options = objectMapper.convertValue(apiSecurityConfig.enableReferrerPolicy(),
					ReferrerPolicyConfig.class);
			middlewares.add(new ReferrerPolicy(options));
		}
	}

	public void processStrictTransportSecurity(List<Filter> middlewares, HttpSecurity http) throws Exception {
		...
		if (apiSecurityConfig.enableStrictTransportSecurity() instanceof Boolean) {
			boolean optionValue = (boolean) apiSecurityConfig.enableStrictTransportSecurity();
			if (optionValue) {
				middlewares.add(new StrictTransportSecurity());
			}
		} else {
			StrictTransportSecurityConfig options = objectMapper.convertValue(
					apiSecurityConfig.enableStrictTransportSecurity(), StrictTransportSecurityConfig.class);
			middlewares.add(new StrictTransportSecurity(options));
		}
	}

	public void processXContentTypeOptions(List<Filter> middlewares, HttpSecurity http) throws Exception {
		// this is for disabling default spring security behaviour
		http.headers().contentTypeOptions().disable();
		if (apiSecurityConfig.enableXContentTypeOptions() == null || apiSecurityConfig.enableXContentTypeOptions()) {
	...
	public void processXDnsPrefetchControl(List<Filter> middlewares) {
		if (apiSecurityConfig.enableXDnsPrefetchControl() instanceof Boolean) {
			boolean optionValue = (boolean) apiSecurityConfig.enableXDnsPrefetchControl();
			if (optionValue) {
				middlewares.add(new XDnsPrefetchControl());
			}
		} else {
			XDnsPrefetchControlConfig options = objectMapper.convertValue(apiSecurityConfig.enableXDnsPrefetchControl(),
	...
	public void processXDownloadOptions(List<Filter> middlewares) {
		if (apiSecurityConfig.enableXDownloadOptions() == null || apiSecurityConfig.enableXDownloadOptions()) {
			middlewares.add(new XDownloadOptions());
		}
	}

	public void processXFrameOptions(List<Filter> middlewares, HttpSecurity http) throws Exception {
		// this is for disabling default spring security behaviour
		...
		if (apiSecurityConfig.enableXFrameOptions() instanceof Boolean) {
			...
			XFrameOptionsConfig options = objectMapper.convertValue(apiSecurityConfig.enableXFrameOptions(),
	...
	public void processXPermittedCrossDomainPolicies(List<Filter> middlewares) {
		if (apiSecurityConfig.enableXPermittedCrossDomainPolicies() instanceof Boolean) {
			boolean optionValue = (boolean) apiSecurityConfig.enableXPermittedCrossDomainPolicies();
			if (optionValue) {
				middlewares.add(new XPermittedCrossDomainPolicies());
			}
		} else {
			XPermittedCrossDomainPoliciesConfig options = objectMapper.convertValue(
					apiSecurityConfig.enableXPermittedCrossDomainPolicies(), XPermittedCrossDomainPoliciesConfig.class);
			middlewares.add(new XPermittedCrossDomainPolicies(options));
		}
	}

	public void processXXssProtections(List<Filter> middlewares, HttpSecurity http) throws Exception {
		// this is for disabling default spring security behaviour
		http.headers().xssProtection().disable();
		if (apiSecurityConfig.enableXXssProtection() == null || apiSecurityConfig.enableXXssProtection()) {
			middlewares.add(new XXssProtection());
		}
	}

	public void processXPoweredBy(HttpSecurity http) {
		if (apiSecurityConfig.enableXPoweredBy() == null || apiSecurityConfig.enableXPoweredBy()) {
			// middlewares.add(new XPoweredBy());

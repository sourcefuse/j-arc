...
public class InvitationExampleController {
  ...
  public ResponseEntity<Invitation> sendInvitationLink(
    @Valid @RequestBody Invitation invitation
  ) {
    log.info(
      "Called by Facade service to save invitation records  and send invitation "
    );
  ...
  public ResponseEntity<ValidateDto> validateInvitationLink(
    ...
    Invitation invitation = invitationRepository
      ...
      .orElseThrow(() ->
        new ResponseStatusException(
          ...
          "Records does not exits against Id" + id
        )
      );
    return new ResponseEntity<>(
      ValidateDto
        .builder()
        .isValid(CommonUtils.calculateTimeDiff(invitation.getExpires()))
        .build(),
      HttpStatus.CREATED
    );
  }

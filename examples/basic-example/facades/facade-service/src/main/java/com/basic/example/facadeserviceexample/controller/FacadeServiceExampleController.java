...
public class FacadeServiceExampleController {
	...
	public Mono<Notification> callEndpoints(@Valid @RequestBody UserDto userDto,
		...
		return webClient.post().uri("http://localhost:8084/tenants/{id}/users", userDto.getTenantId())
				...
				.flatMap(invitation -> sendNotification(userDto.getUserDetails(), invitation.getId().toString(),
						finalBearerToken, currentUser));
	}
	...
	private Mono<Invitation> sendInvitation(UserDto userDto, String finalBearerToken) {
	...
	private Mono<Notification> sendNotification(User userDetails, String invitationId, String finalBearerToken,
			CurrentUser currentUser) {

		String link = feUrl + "invitation/" + invitationId;
		String emailBody = EmailConstants.INVITATION_MAIL_TEMPLATE.replace("{USER_NAME}",
				currentUser.getFirstName() + " " + currentUser.getLastName()).replace("{INVITATION_LINK}", link);

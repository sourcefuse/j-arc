...
public final class CustomOAuth2AuthorizationRequestResolver
  ...
  private static final String REGISTRATION_ID_URI_VARIABLE_NAME =
    "registrationId";

  private static final char PATH_DELIMITER = '/';

  private static final StringKeyGenerator DEFAULT_SECURE_KEY_GENERATOR =
    new Base64StringKeyGenerator(Base64.getUrlEncoder().withoutPadding(), 96);

  private static final Consumer<
    OAuth2AuthorizationRequest.Builder
  > DEFAULT_PKCE_APPLIER = OAuth2AuthorizationRequestCustomizers.withPkce();
  ...
  public CustomOAuth2AuthorizationRequestResolver(
    ...
    Assert.hasText(
      ...
      "authorizationRequestBaseUri cannot be empty"
    );
    this.clientRegistrationRepository = clientRegistrationRepository;
    this.authorizationRequestMatcher =
      new AntPathRequestMatcher(
   ...
   * {@link OAuth2AuthorizationRequest.Builder} allowing for further
   * customizations.
   *
  ...
  private static OAuth2AuthorizationRequest.Builder getBuilder(
    ...
    if (
      ...
      if (
        !CollectionUtils.isEmpty(clientRegistration.getScopes()) &&
  ...
  /**
   * Expands the {@link ClientRegistration#getRedirectUri()} with following
   * provided variables:<br/>
   ...
   * Requests.
   *
   * @param builder where the {@link OidcParameterNames#NONCE} and hash is stored

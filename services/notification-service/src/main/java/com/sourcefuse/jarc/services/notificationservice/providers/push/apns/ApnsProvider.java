...
public class ApnsProvider implements PushNotification {
	...
	private void initialValidations(Message message) {
		if (message.getOptions().get("messageFrom") == null
	...
	private String getMainNote(Message message) {
		int defaultBadgeCount = Optional.ofNullable(apnsConnection.getBadge()).orElse(3);

		Map<String, Object> cutomFields = new HashMap<>();
		cutomFields.put("messageFrom", message.getOptions().get("messageFrom"));
		cutomFields.put("messageFrom", message.getOptions().get("messageFrom"));

		String payload = APNS.newPayload().alertBody(message.getBody()).alertTitle(message.getSubject())
				// The category/ topic is usually the bundle identifier of your application.
				.category(this.apnsConnection.getTopic()).customFields(cutomFields).badge(defaultBadgeCount).build();
		return payload;
	}
	...
	private void sendingPushToReceiverTokens(Message message) {
		...
		List<Subscriber> receiverTokens = message.getReceiver().getTo().stream().filter(item -> item.getType() == null
				|| item.getType().toString().equals(ApnsSubscriberType.RegistrationToken.toString())).toList();
		if (receiverTokens.size() >= 1) {
			List<String> tokens = receiverTokens.stream().map(item -> item.getId()).toList();
			this.apnsConnection.getApnsService().push(tokens, this.getMainNote(message), expiresDate);
		}
	}

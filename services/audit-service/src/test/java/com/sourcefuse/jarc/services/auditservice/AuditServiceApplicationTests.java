...
public class AuditServiceApplicationTests {
	...
	public void shouldFailedDueToWrongActionIsProvided() throws Exception {
		...
		requestBody = requestBody.replace("\"action\":null", "\"action\":\"SAVEALL\"");

		this.mockMvc.perform(post("/audit_logs").contentType("application/json").content(requestBody))
				.andExpect(status().isBadRequest());
	}
	...
	public void shouldFailedDueToWrongActedAtIsProvided() throws Exception {
		auditLog.setActedAt(null);
		String requestBody = objectMapper.writeValueAsString(auditLog);
		requestBody = requestBody.replace("\"action\":null", "\"actedAt\":\"12/12/2023\"");
	...
	public void testGetAuditLogCount() throws Exception {
		this.mockMvc.perform(get("/audit_logs/count")).andExpect(status().isOk())
				.andExpect(content().string(equalTo("2")));
	}

	@Test
	public void testGetAllAuditLogs() throws Exception {
		MvcResult mvcResult = this.mockMvc.perform(get("/audit_logs")).andExpect(status().isOk()).andReturn();
		String actualResponseBody = mvcResult.getResponse().getContentAsString();

		List<AuditLog> responseAuditLogs = objectMapper.readValue(actualResponseBody,
	...
	public void testGetAuditLogById() throws Exception {
		MvcResult mvcResult = this.mockMvc.perform(get("/audit_logs/" + auditLog1.getId())).andExpect(status().isOk())
				.andReturn();
		String actualResponseBody = mvcResult.getResponse().getContentAsString();
	...
	private void setAuditLog1() {
		...
		auditLog1.setEntityId(UUID.fromString("c87a2848-ce70-41a9-95c7-d7ab230ebcac"));
	...
	private void setAuditLog2() {
		...
		auditLog2.setEntityId(UUID.fromString("c87a2848-ce70-41a9-95c7-d7ab230ebcac"));
	...
	private void setAuditLog() {
		...
		auditLog.setBefore(
				"{\"id\": \"c87a2848-ce70-41a9-95c7-d7ab230ebcac\", \"name\": \"Temp Updated\", \"deleted\": false, \"permissons\": \"Get\"}");
		auditLog.setEntityId(UUID.fromString("c87a2848-ce70-41a9-95c7-d7ab230ebcac"));
